...
import (
	...
	"tennet/gethired/application/security/jwt"
	errorsDomain "tennet/gethired/domain/errors"
...
func (s *Service) Login(user LoginUser) (*SecurityAuthenticatedUser, error) {
	...
	domainUserRole, err := s.UserRepository.GetOneByMap(userMap)
	if err != nil {
	...
	accessTokenClaims, err := jwt.GenerateJWTToken(domainUserRole.ID, "access")
	if err != nil {
		return &SecurityAuthenticatedUser{}, err
	}
	refreshTokenClaims, err := jwt.GenerateJWTToken(domainUserRole.ID, "refresh")
	if err != nil {
		return &SecurityAuthenticatedUser{}, err
	}

	return secAuthUserMapper(domainUserRole, &Auth{
